...
func InitLogger() {
	...
	// 设置初始化字段
	filed := zap.Fields(zap.String("service_name", config.GetServer().Name))
	// 构造日志
...
func (w Writer) Write(p []byte) (n int, err error) {
	...
	str := fmt.Sprintf(`{"level":"%v","time":"%v","msg":"%v","service_name":"%v","category":"%v","line":" %s "} `, tmpLog.Level, tmpLog.Time, tmpLog.Msg, tmpLog.ServiceName, tmpLog.Category, tmpLog.Line)
	fmt.Println(str)
	return
}
